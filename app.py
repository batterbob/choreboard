...
def inject_globals():
    ...
    return {
        "app_name": g_set("app_name", "Choreify"),
...
def settings_view(conn):
    ...
    return {
        ...
        "app_name_val": g("app_name", "Choreify"),
...
def admin_settings_notify_test():
    conn = get_db()
    app_name = logic.get_setting(conn, "app_name", "Choreify")
...
def api_v1_status():
    """Machine-readable daily status for Tidbyt, Home Assistant, and other integrations.

    Public, read-only, no auth — exposes kid names and chore names only.
    Schema: new fields may be added; existing keys will not be removed or renamed.
    """
    conn = get_db()
    d = effective_today()
    logic.finalize_past_weeks(conn, d)
    ...
    ws = logic.week_start(d)
    in_prog = logic.in_program_window(conn, d)
    paused = logic.is_paused(conn, d)
    acts = logic.activities(conn, enabled_only=True)
    ...
    for kid in logic.active_kids(conn):
        ...
        done_map = logic.completed_chore_ids(conn, kid["id"], d)
        checklist_done, completed_at = logic.checklist_status(conn, kid["id"], d)
        acts_data = [activity_section(conn, kid, a, d) for a in acts]
        stars, streak = logic.scoreboard(conn, kid["id"])
        banner = logic.banner_state(conn, kid, d)
        kids_out.append(_kid_status_dict(
            kid, daily, done_map, checklist_done, completed_at,
            acts_data, stars, streak, banner))

    return jsonify({
        "generated_at": logic.now_iso(),
        "date": logic.d2s(d),
        "program": {
            "active": in_prog,
            "paused": paused,
            "start": logic.get_setting(conn, "program_start_date", ""),
            "end": logic.get_setting(conn, "program_end_date", ""),
        },
        "kids": kids_out,
    })


@app.route("/api/v1/status/<slug>")
def api_v1_status_kid(slug):
    """Same schema as /api/v1/status but scoped to one kid — useful for per-kid displays."""
    conn = get_db()
    ...
    return jsonify({
        ...
        "kid": _kid_status_dict(
            kid, daily, done_map, checklist_done, completed_at,
            acts_data, stars, streak, banner),
    })


def _kid_status_dict(kid, daily, done_map, checklist_done, completed_at,
                     acts_data, stars, streak, banner):
    return {
        "id": kid["id"],
        "name": kid["name"],
        ...
        "checklist": {
            ...
            "completed_at": completed_at,
            "total": len(daily),
            "finished": sum(1 for c in daily if c["id"] in done_map),
            "chores": [{"id": c["id"], "name": c["name"],
                        "done": c["id"] in done_map} for c in daily],
        },
        "activities": [
            {
                "key": s["kind"],
                "label": s["label"],
                "unit": s["unit"],
                "week_amount": s["weekly"],
                "target": s["target"],
                "pct": _pct(s["weekly"], s["target"]),
                "met": s["met"],
            }
            for s in acts_data
        ],
        "banner": banner["state"],
        "bonus_earned": banner["state"] == "earned",
        "stars": stars,
        "streak": streak,
    }
...
def setup_wizard():
    ...
    if not is_first_run(conn):
        return redirect("/admin")
    if request.method == "POST":
        app_name = (request.form.get("app_name") or "Choreify").strip()
...
def admin_export():
    ...
    return Response(payload, mimetype="application/json",
                    headers={"Content-Disposition": "attachment; filename=choreify_export.json"})
